...
RUN apt-get update && apt-get install -y python python3 python3-setuptools libcurl4-gnutls-dev libpython3-dev
...
RUN R --vanilla -e "\
  ...
  sapply(c('rhdf5', 'dplyr', 'tidyr', 'reshape2', 'pheatmap', 'corrplot', \
           ...
           'ggpubr', 'forcats', 'Rtsne', 'uwot', 'MultiAssayExperiment', 'Biobase'), \
         BiocManager::install)"
